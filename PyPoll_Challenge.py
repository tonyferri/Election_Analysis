...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_list:
        ...
        votecount_county = county_votes[county]
        # 6c: Calculate the percentage of votes for the county.
        county_perc = (float(votecount_county) / float(total_votes)) * 100
        # 6d: Print the county results to the terminal.
        county_results = (
            f"{county}: {county_perc:.1f}% ({votecount_county:,})")
        print(county_results)
        # 6e: Save the county votes to a text file.
        county_votes_txt = (f"{county}: {county_perc:.1f}% ({votecount_county:,})\n")
        txt_file.write(county_votes_txt)
        # 6f: Write an if statement to determine the winning county and get it's vote count.
        if (votecount_county > turnout_voters):
            turnout_county = county
            turnout_voters = votecount_county
